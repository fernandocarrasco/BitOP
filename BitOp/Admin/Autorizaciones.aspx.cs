...
namespace BitOp.Admin
{
    public partial class Autorizaciones : System.Web.UI.Page
    {
        ...
        protected void ButtonAgrAll_Click(object sender, EventArgs e)
        {
            ...
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Insert into [dbo].[Autorizaciones] SELECT '" + Perfil.Trim() + "' as [Perfil],[Modulo],[Opcion],'Activo' as [Estado],Null as [Fecha Creación],Null as [Fecha Modif],Null as [Usuario Modif] FROM [BopDB].[dbo].[Opciones]";
            cmd.ExecuteNonQuery();
            con.Close();
            GridViewAutAsig.DataBind();
        }
